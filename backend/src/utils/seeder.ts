...
export const seedDatabase = async () => {
  ...
  await About.create(initialAboutData);
  await Skill.insertMany(initialSkillsData);
  await Education.insertMany(initialEducationData);
  await Project.insertMany(initialProjectsData);
  await Travel.insertMany(initialTravelData);
  await Social.insertMany(initialSocialsData);
  await ContactInfo.create(initialContactDetails);
  await ContactMessage.insertMany(initialMessagesData);
  await Settings.create(initialSettingsData);
  await ActivityLog.insertMany(initialActivitiesData);
