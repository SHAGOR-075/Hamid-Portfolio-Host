...
export const sendContactNotificationEmail = async (payload: EmailPayload): Promise<boolean> => {
  ...
  if (!smtpUser || !smtpPass) {
    console.log(
      `[Email Notification]: SMTP_USER / SMTP_PASS not set in .env. Form submission from ${name} (${email}) saved to MongoDB inbox.`
    );
    return false;
  }
