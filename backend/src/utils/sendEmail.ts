...
export const sendContactNotificationEmail = async (
  ...
  if (!isEmailConfigured()) {
    console.error(
      `[Email Notification]: SMTP_USER / SMTP_PASS missing. Message from ${name} (${email}) saved, but email was not sent.`
    );
    return false;
  }
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      console.log(
        `[Email Notification]: Successfully sent notification email to ${receiverEmail}`
      );
      return true;
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      console.error(
        `[Email Notification Error]: Attempt ${attempt} failed to send email:`,
        message
      );
